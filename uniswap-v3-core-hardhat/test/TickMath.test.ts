...
describe("TickMath", () => {
  ...
  before("deploy TickMathTest", async () => {
    tickMath = (await ethers.deployContract(
      "TickMathTest"
    )) as unknown as TickMathTest;
    await tickMath.waitForDeployment();
  });

  describe("#getSqrtRatioAtTick", () => {
    it("throws for too low", async () => {
      await expect(tickMath.getSqrtRatioAtTick(MIN_TICK - 1)).to.be.reverted;
    });

    it("throws for too high", async () => {
      await expect(tickMath.getSqrtRatioAtTick(MAX_TICK + 1)).to.be.reverted;
    });
  ...
  describe("#getTickAtSqrtRatio", () => {
    it("throws for too low", async () => {
      await expect(tickMath.getTickAtSqrtRatio(MIN_SQRT_RATIO - 1n)).to.be
        .reverted;
    });

    it("throws for too high", async () => {
      await expect(tickMath.getTickAtSqrtRatio(MAX_SQRT_RATIO)).to.be.reverted;
    });
    ...
    it("ratio closest to max tick", async () => {
      expect(await tickMath.getTickAtSqrtRatio(MAX_SQRT_RATIO - 1n)).to.eq(
        MAX_TICK - 1
      );
    });
