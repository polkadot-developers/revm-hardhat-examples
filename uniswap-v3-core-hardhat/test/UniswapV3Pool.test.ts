...
import { MockTimeUniswapV3Pool } from '../typechain-types/MockTimeUniswapV3Pool'
import { TestUniswapV3SwapPay } from '../typechain-types/TestUniswapV3SwapPay'
...
import { TestUniswapV3Callee } from '../typechain-types/TestUniswapV3Callee'
import { TestUniswapV3ReentrantCallee } from '../typechain-types/TestUniswapV3ReentrantCallee'
import { TickMathTest } from '../typechain-types/TickMathTest'
import { SwapMathTest } from '../typechain-types/SwapMathTest'
...
describe('UniswapV3Pool', () => {
  ...
  describe('#mint', () => {
    it('fails if not initialized', async () => {
      await expect(mint(await wallet.getAddress(), -tickSpacing, tickSpacing, 1)).to.be.revertedWith('LOK')
    })
    describe('after initialization', () => {
      ...
      describe('failure cases', () => {
        ...
        it('fails if amount exceeds the max', async () => {
          ...
          await expect(mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, maxLiquidityGross + 1n)).to
            .be.reverted
          await expect(mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, maxLiquidityGross)).to.not.be
            .reverted
        })
        it('fails if total amount at tick exceeds the max', async () => {
          // these should fail with 'LO' but hardhat is bugged
          await mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, 1000)

          const maxLiquidityGross = await pool.maxLiquidityPerTick()
          await expect(
            mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, maxLiquidityGross - 1000n + 1n)
          ).to.be.reverted
          await expect(
            mint(await wallet.getAddress(), minTick + tickSpacing * 2, maxTick - tickSpacing, maxLiquidityGross - 1000n + 1n)
          ).to.be.reverted
          await expect(
            mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing * 2, maxLiquidityGross - 1000n + 1n)
          ).to.be.reverted
          await expect(mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, maxLiquidityGross - 1000n))
            .to.not.be.reverted
        })
        it('fails if amount is 0', async () => {
          await expect(mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, 0)).to.be.reverted
        })
      })
      ...
      describe('success cases', () => {
        ...
        describe('above current price', () => {
          ...
          it('removing works', async () => {
            ...
            const { amount0, amount1 } = await pool.collect.staticCall(await wallet.getAddress(), -240, 0, MaxUint128, MaxUint128)
        ...
        describe('including current price', () => {
          it('price within range: transfers current price of both tokens', async () => {
            await expect(mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, 100))
              .to.emit(token0, 'Transfer')
      ...
      it('protocol fees accumulate as expected during swap', async () => {
        ...
        await mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(1))
        await swapExact0For1(expandTo18Decimals(1) / 10n, await wallet.getAddress())
        await swapExact1For0(expandTo18Decimals(1) / 100n, await wallet.getAddress())
      ...
      it('poke is not allowed on uninitialized position', async () => {
        await mint(await other.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(1))
        await swapExact0For1(expandTo18Decimals(1) / 10n, await wallet.getAddress())
        await swapExact1For0(expandTo18Decimals(1) / 100n, await wallet.getAddress())
        ...
        let {
          ...
          tokensOwed1,
          tokensOwed0,
        } = await pool.positions(getPositionKey(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing))
  ...
  describe('#burn', () => {
    ...
    it('does not clear the position fee growth snapshot if no more liquidity', async () => {
      ...
      await swapExact1For0(expandTo18Decimals(1), await wallet.getAddress())
  ...
  describe('#observe', () => {
    ...
    it('current tick accumulator after two swaps', async () => {
      await swapExact0For1(expandTo18Decimals(1) / 2n, await wallet.getAddress())
      expect((await pool.slot0()).tick).to.eq(-4452)
      await pool.advanceTime(4)
      await swapExact1For0(expandTo18Decimals(1) / 4n, await wallet.getAddress())
  ...
  describe('miscellaneous mint tests', () => {
    ...
    it('collect fees within the current price after swap', async () => {
      ...
      await swapExact0For1(amount0In, await wallet.getAddress())
    ...
    describe('k (implicit)', () => {
      ...
      describe('post initialized', () => {
        ...
        it('excludes supply at tick above current tick', async () => {
          await mint(await wallet.getAddress(), tickSpacing, tickSpacing * 2, expandTo18Decimals(3))
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(2))
        })
        it('excludes supply at tick below current tick', async () => {
          await mint(await wallet.getAddress(), -tickSpacing * 2, -tickSpacing, expandTo18Decimals(3))
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(2))
        })
        it('updates correctly when exiting range', async () => {
          ...
          await swapExact0For1(1, await wallet.getAddress())
  ...
  describe('limit orders', () => {
    ...
    it('limit selling 0 for 1 at tick 0 thru 1', async () => {
      await expect(mint(await wallet.getAddress(), 0, 120, expandTo18Decimals(1)))
        .to.emit(token0, 'Transfer')
        .withArgs(await wallet.getAddress(), await pool.getAddress(), '5981737760509663')
      // somebody takes the limit order
      await swapExact1For0(expandTo18Decimals(2), await other.getAddress())
      await expect(pool.burn(0, 120, expandTo18Decimals(1)))
        .to.emit(pool, 'Burn')
        .withArgs(await wallet.getAddress(), 0, 120, expandTo18Decimals(1), 0, '6017734268818165')
        .to.not.emit(token0, 'Transfer')
        .to.not.emit(token1, 'Transfer')
      await expect(pool.collect(await wallet.getAddress(), 0, 120, MaxUint128, MaxUint128))
        .to.emit(token1, 'Transfer')
        .withArgs(await pool.getAddress(), await wallet.getAddress(), 6017734268818165n + 18107525382602n) // roughly 0.3% despite other liquidity
        .to.not.emit(token0, 'Transfer')
    ...
    it('limit selling 1 for 0 at tick 0 thru -1', async () => {
      await expect(mint(await wallet.getAddress(), -120, 0, expandTo18Decimals(1)))
        ...
        .withArgs(await wallet.getAddress(), await pool.getAddress(), '5981737760509663')
      // somebody takes the limit order
      await swapExact0For1(expandTo18Decimals(2), await other.getAddress())
      await expect(pool.burn(-120, 0, expandTo18Decimals(1)))
        .to.emit(pool, 'Burn')
        .withArgs(await wallet.getAddress(), -120, 0, expandTo18Decimals(1), '6017734268818165', 0)
        .to.not.emit(token0, 'Transfer')
        .to.not.emit(token1, 'Transfer')
    ...
    describe('fee is on', () => {
      ...
      it('limit selling 0 for 1 at tick 0 thru 1', async () => {
        ...
        await expect(pool.collect(await wallet.getAddress(), 0, 120, MaxUint128, MaxUint128))
          .to.emit(token1, 'Transfer')
          .withArgs(await pool.getAddress(), await wallet.getAddress(), 6017734268818165n + 15089604485501n) // roughly 0.25% despite other liquidity
          .to.not.emit(token0, 'Transfer')
        expect((await pool.slot0()).tick).to.be.gte(120)
      })
      it('limit selling 1 for 0 at tick 0 thru -1', async () => {
  ...
  describe('#collect', () => {
    ...
    it('works with multiple LPs', async () => {
      ...
      await mint(await wallet.getAddress(), minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(2))
    ...
    describe('works across large increases', () => {
      ...
      it('works just before the cap binds', async () => {
        ...
        const { tokensOwed0, tokensOwed1 } = await pool.positions(getPositionKey(await wallet.getAddress(), minTick, maxTick))
  ...
  describe('#feeProtocol', () => {
    ...
    beforeEach(async () => {
      pool = await createPool(FeeAmount.LOW, TICK_SPACINGS[FeeAmount.LOW])
      await pool.initialize(encodePriceSqrt(1, 1))
      await mint(await wallet.getAddress(), minTick, maxTick, liquidityAmount)
    })
    ...
    async function swapAndGetFeesOwed({
      ...
      await (zeroForOne ? swapExact0For1(amount, await wallet.getAddress()) : swapExact1For0(amount, await wallet.getAddress()))
    ...
    describe('#collectProtocol', () => {
      it('returns 0 if no fees', async () => {
        await pool.setFeeProtocol(6, 6)
        const { amount0, amount1 } = await pool.collectProtocol.staticCall(await wallet.getAddress(), MaxUint128, MaxUint128)
        expect(amount0).to.be.eq(0)
        expect(amount1).to.be.eq(0)
      })
      ...
      it('can collect fees', async () => {
        ...
        await expect(pool.collectProtocol(await other.getAddress(), MaxUint128, MaxUint128))
          .to.emit(token0, 'Transfer')
          .withArgs(await pool.getAddress(), await other.getAddress(), '83333333333332')
      })
      ...
      it('fees collected can differ between token0 and token1', async () => {
        ...
        await expect(pool.collectProtocol(await other.getAddress(), MaxUint128, MaxUint128))
          ...
          .withArgs(await pool.getAddress(), await other.getAddress(), '62499999999999')
          .to.emit(token1, 'Transfer')
          // less token1 fees because it's 1/8th the swap fees
          .withArgs(await pool.getAddress(), await other.getAddress(), '99999999999998')
      })
    })
    ...
    it('fees collected by lp after two swaps with intermediate withdrawal', async () => {
      ...
      // collect the fees
      await pool.collect(await wallet.getAddress(), minTick, maxTick, MaxUint128, MaxUint128)
      ...
      await expect(pool.collect(await wallet.getAddress(), minTick, maxTick, MaxUint128, MaxUint128))
        .to.emit(token0, 'Transfer')
        .withArgs(await pool.getAddress(), await wallet.getAddress(), '416666666666666')
      ;({ token0: token0ProtocolFees, token1: token1ProtocolFees } = await pool.protocolFees())
      expect(token0ProtocolFees).to.eq('166666666666666')
      expect(token1ProtocolFees).to.eq(0)
    })
    ...
    describe('tickSpacing = 12', () => {
      ...
      describe('post initialize', () => {
        ...
        it('mint can only be called for multiples of 12', async () => {
          await expect(mint(await wallet.getAddress(), -6, 0, 1)).to.be.reverted
          await expect(mint(await wallet.getAddress(), 0, 6, 1)).to.be.reverted
        })
        it('mint can be called with multiples of 12', async () => {
          await mint(await wallet.getAddress(), 12, 24, 1)
          await mint(await wallet.getAddress(), -144, -120, 1)
        })
        it('swapping across gaps works in 1 for 0 direction', async () => {
          const liquidityAmount = expandTo18Decimals(1) / 4n
          await mint(await wallet.getAddress(), 120000, 121200, liquidityAmount)
          await swapExact1For0(expandTo18Decimals(1), await wallet.getAddress())
          await expect(pool.burn(120000, 121200, liquidityAmount))
            .to.emit(pool, 'Burn')
            .withArgs(await wallet.getAddress(), 120000, 121200, liquidityAmount, '30027458295511', '996999999999999999')
            .to.not.emit(token0, 'Transfer')
            .to.not.emit(token1, 'Transfer')
        ...
        it('swapping across gaps works in 0 for 1 direction', async () => {
          const liquidityAmount = expandTo18Decimals(1) / 4n
          await mint(await wallet.getAddress(), -121200, -120000, liquidityAmount)
          await swapExact0For1(expandTo18Decimals(1), await wallet.getAddress())
          await expect(pool.burn(-121200, -120000, liquidityAmount))
            .to.emit(pool, 'Burn')
            .withArgs(await wallet.getAddress(), -121200, -120000, liquidityAmount, '996999999999999999', '30027458295511')
            .to.not.emit(token0, 'Transfer')
            .to.not.emit(token1, 'Transfer')
  ...
  it('tick transition cannot run twice if zero for one swap ends at fractional price just below tick', async () => {
    ...
    await mint(await wallet.getAddress(), -24082, -24080, liquidity)
    expect(await pool.liquidity(), 'current pool liquidity is now liquidity + 1').to.eq(liquidity)

    await mint(await wallet.getAddress(), -24082, -24081, liquidity)
    ...
    await expect(swapExact0For1(3, await wallet.getAddress()))
      .to.emit(token0, 'Transfer')
      .withArgs(await wallet.getAddress(), await pool.getAddress(), 3)
      .to.not.emit(token1, 'Transfer')
  ...
  describe('#flash', () => {
    it('fails if not initialized', async () => {
      await expect(flash(100, 200, await other.getAddress())).to.be.reverted
      await expect(flash(100, 0, await other.getAddress())).to.be.reverted
      await expect(flash(0, 200, await other.getAddress())).to.be.reverted
    })
    it('fails if no liquidity', async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
      await expect(flash(100, 200, await other.getAddress())).to.be.revertedWith('L')
      await expect(flash(100, 0, await other.getAddress())).to.be.revertedWith('L')
      await expect(flash(0, 200, await other.getAddress())).to.be.revertedWith('L')
    })
    describe('after liquidity added', () => {
      ...
      beforeEach('add some tokens', async () => {
        await initializeAtZeroTick(pool)
        ;[balance0, balance1] = await Promise.all([token0.balanceOf(await pool.getAddress()), token1.balanceOf(await pool.getAddress())])
      })

      describe('fee off', () => {
        it('emits an event', async () => {
          await expect(flash(1001, 2001, await other.getAddress()))
            .to.emit(pool, 'Flash')
            .withArgs(await swapTarget.getAddress(), await other.getAddress(), 1001, 2001, 4, 7)
        })

        it('transfers the amount0 to the recipient', async () => {
          await expect(flash(100, 200, await other.getAddress()))
            .to.emit(token0, 'Transfer')
            .withArgs(await pool.getAddress(), await other.getAddress(), 100)
        })
        it('transfers the amount1 to the recipient', async () => {
          await expect(flash(100, 200, await other.getAddress()))
            .to.emit(token1, 'Transfer')
            .withArgs(await pool.getAddress(), await other.getAddress(), 200)
        })
        it('can flash only token0', async () => {
          await expect(flash(101, 0, await other.getAddress()))
            .to.emit(token0, 'Transfer')
            .withArgs(await pool.getAddress(), await other.getAddress(), 101)
            .to.not.emit(token1, 'Transfer')
        })
        it('can flash only token1', async () => {
          await expect(flash(0, 102, await other.getAddress()))
            .to.emit(token1, 'Transfer')
            .withArgs(await pool.getAddress(), await other.getAddress(), 102)
            .to.not.emit(token0, 'Transfer')
        })
        it('can flash entire token balance', async () => {
          await expect(flash(balance0, balance1, await other.getAddress()))
            .to.emit(token0, 'Transfer')
            .withArgs(await pool.getAddress(), await other.getAddress(), balance0)
            .to.emit(token1, 'Transfer')
            .withArgs(await pool.getAddress(), await other.getAddress(), balance1)
        })
        it('no-op if both amounts are 0', async () => {
          await expect(flash(0, 0, await other.getAddress())).to.not.emit(token0, 'Transfer').to.not.emit(token1, 'Transfer')
        })
        it('fails if flash amount is greater than token balance', async () => {
          await expect(flash(balance0 + 1n, balance1, await other.getAddress())).to.be.reverted
          await expect(flash(balance0, balance1 + 1n, await other.getAddress())).to.be.reverted
        })
        it('calls the flash callback on the sender with correct fee amounts', async () => {
          await expect(flash(1001, 2002, await other.getAddress())).to.emit(swapTarget, 'FlashCallback').withArgs(4, 7)
        })
        it('increases the fee growth by the expected amount', async () => {
          await flash(1001, 2002, await other.getAddress())
          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            4n * 2n ** 128n / expandTo18Decimals(2)
          )
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            7n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
        it('fails if original balance not returned in either token', async () => {
          await expect(flash(1000, 0, await other.getAddress(), 999, 0)).to.be.reverted
          await expect(flash(0, 1000, await other.getAddress(), 0, 999)).to.be.reverted
        })
        it('fails if underpays either token', async () => {
          await expect(flash(1000, 0, await other.getAddress(), 1002, 0)).to.be.reverted
          await expect(flash(0, 1000, await other.getAddress(), 0, 1002)).to.be.reverted
        })
        it('allows donating token0', async () => {
          await expect(flash(0, 0, ethers.ZeroAddress, 567, 0))
            .to.emit(token0, 'Transfer')
            .withArgs(await wallet.getAddress(), await pool.getAddress(), 567)
            .to.not.emit(token1, 'Transfer')
          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            567n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
        it('allows donating token1', async () => {
          await expect(flash(0, 0, ethers.ZeroAddress, 0, 678))
            .to.emit(token1, 'Transfer')
            .withArgs(await wallet.getAddress(), await pool.getAddress(), 678)
            .to.not.emit(token0, 'Transfer')
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            678n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
        it('allows donating token0 and token1 together', async () => {
          await expect(flash(0, 0, ethers.ZeroAddress, 789, 1234))
            .to.emit(token0, 'Transfer')
            .withArgs(await wallet.getAddress(), await pool.getAddress(), 789)
            .to.emit(token1, 'Transfer')
            .withArgs(await wallet.getAddress(), await pool.getAddress(), 1234)

          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            789n * 2n ** 128n / expandTo18Decimals(2)
          )
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            1234n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
      })
      ...
      describe('fee on', () => {
        ...
        it('increases the fee growth by the expected amount', async () => {
          await flash(2002, 4004, await other.getAddress())
          ...
          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            6n * 2n ** 128n / expandTo18Decimals(2)
          )
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            11n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
        it('allows donating token0', async () => {
          await expect(flash(0, 0, ethers.ZeroAddress, 567, 0))
          ...
          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            473n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
        it('allows donating token1', async () => {
          await expect(flash(0, 0, ethers.ZeroAddress, 0, 678))
          ...
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            565n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
        it('allows donating token0 and token1 together', async () => {
          await expect(flash(0, 0, ethers.ZeroAddress, 789, 1234))
          ...
          expect(await pool.feeGrowthGlobal0X128()).to.eq(
            658n * 2n ** 128n / expandTo18Decimals(2)
          )
          expect(await pool.feeGrowthGlobal1X128()).to.eq(
            1029n * 2n ** 128n / expandTo18Decimals(2)
          )
        })
      })
    })
  ...
  describe('#snapshotCumulativesInside', () => {
    ...
    beforeEach(async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
      await mint(await wallet.getAddress(), tickLower, tickUpper, 10)
    })
    ...
    it('does not account for time increase above range', async () => {
      await pool.advanceTime(5)
      await swapToHigherPrice(encodePriceSqrt(2, 1), await wallet.getAddress())
    ...
    it('does not account for time increase below range', async () => {
      await pool.advanceTime(5)
      await swapToLowerPrice(encodePriceSqrt(1, 2), await wallet.getAddress())
      await pool.advanceTime(7)
      const {
        secondsPerLiquidityInsideX128,
        tickCumulativeInside,
    ...
    it('time increase below range is not counted', async () => {
      await swapToLowerPrice(encodePriceSqrt(1, 2), await wallet.getAddress())
      await pool.advanceTime(5)
      await swapToHigherPrice(encodePriceSqrt(1, 1), await wallet.getAddress())
      await pool.advanceTime(7)
      const {
        secondsPerLiquidityInsideX128,
        tickCumulativeInside,
    ...
    it('time increase above range is not counted', async () => {
      await swapToHigherPrice(encodePriceSqrt(2, 1), await wallet.getAddress())
      await pool.advanceTime(5)
      await swapToLowerPrice(encodePriceSqrt(1, 1), await wallet.getAddress())
      await pool.advanceTime(7)
      const {
        secondsPerLiquidityInsideX128,
        tickCumulativeInside,
    ...
    it('positions minted after time spent', async () => {
      await pool.advanceTime(5)
      await mint(await wallet.getAddress(), tickUpper, getMaxTick(tickSpacing), 15)
      await swapToHigherPrice(encodePriceSqrt(2, 1), await wallet.getAddress())
    ...
    it('overlapping liquidity is aggregated', async () => {
      await mint(await wallet.getAddress(), tickLower, getMaxTick(tickSpacing), 15)
      await pool.advanceTime(5)
      await swapToHigherPrice(encodePriceSqrt(2, 1), await wallet.getAddress())
      await pool.advanceTime(8)
      const {
    ...
    it('relative behavior of snapshots', async () => {
      await pool.advanceTime(5)
      await mint(await wallet.getAddress(), getMinTick(tickSpacing), tickLower, 15)
      const {
  ...
  describe('fees overflow scenarios', async () => {
    it('up to max uint 128', async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
      await mint(await wallet.getAddress(), minTick, maxTick, 1)
      await flash(0, 0, await wallet.getAddress(), MaxUint128, MaxUint128)
    ...
    it('overflow max uint 128', async () => {
      ...
      await mint(await wallet.getAddress(), minTick, maxTick, 1)
      await flash(0, 0, await wallet.getAddress(), MaxUint128, MaxUint128)
      await flash(0, 0, await wallet.getAddress(), 1, 1)
    ...
    it('two positions at the same snapshot', async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
      await mint(await wallet.getAddress(), minTick, maxTick, 1)
      await mint(await other.getAddress(), minTick, maxTick, 1)
      await flash(0, 0, await wallet.getAddress(), MaxUint128, 0)
      await flash(0, 0, await wallet.getAddress(), MaxUint128, 0)
      const feeGrowthGlobal0X128 = await pool.feeGrowthGlobal0X128()
      expect(feeGrowthGlobal0X128).to.eq(MaxUint128 << 128n)
      await flash(0, 0, await wallet.getAddress(), 2, 0)
      await pool.burn(minTick, maxTick, 0)
      await pool.connect(other).burn(minTick, maxTick, 0)
      let { amount0 } = await pool.collect.staticCall(await wallet.getAddress(), minTick, maxTick, MaxUint128, MaxUint128)
      expect(amount0, 'amount0 of wallet').to.eq(0)
      ;({ amount0 } = await pool
        .connect(other)
        .collect.staticCall(await other.getAddress(), minTick, maxTick, MaxUint128, MaxUint128))
    ...
    it('two positions 1 wei of fees apart overflows exactly once', async () => {
      await pool.initialize(encodePriceSqrt(1, 1))
      await mint(await wallet.getAddress(), minTick, maxTick, 1)
      await flash(0, 0, await wallet.getAddress(), 1, 0)
      await mint(await other.getAddress(), minTick, maxTick, 1)
      await flash(0, 0, await wallet.getAddress(), MaxUint128, 0)
      await flash(0, 0, await wallet.getAddress(), MaxUint128, 0)
      const feeGrowthGlobal0X128 = await pool.feeGrowthGlobal0X128()
  ...
  describe('swap underpayment tests', () => {
    ...
    it('underpay zero for one and exact in', async () => {
      await expect(
        underpay.swap(await pool.getAddress(), await wallet.getAddress(), true, MIN_SQRT_RATIO + 1n, 1000, 1, 0)
      ).to.be.revertedWith('IIA')
    })
    it('pay in the wrong token zero for one and exact in', async () => {
      await expect(
        underpay.swap(await pool.getAddress(), await wallet.getAddress(), true, MIN_SQRT_RATIO + 1n, 1000, 0, 2000)
      ).to.be.revertedWith('IIA')
    })
    it('overpay zero for one and exact in', async () => {
      await expect(
        underpay.swap(await pool.getAddress(), await wallet.getAddress(), true, MIN_SQRT_RATIO + 1n, 1000, 2000, 0)
    ...
    it('underpay one for zero and exact in', async () => {
      await expect(
        underpay.swap(await pool.getAddress(), await wallet.getAddress(), false, MAX_SQRT_RATIO - 1n, 1000, 0, 1)
      ).to.be.revertedWith('IIA')
    })
    it('pay in the wrong token one for zero and exact in', async () => {
      await expect(
        underpay.swap(await pool.getAddress(), await wallet.getAddress(), false, MAX_SQRT_RATIO - 1n, 1000, 2000, 0)
      ).to.be.revertedWith('IIA')
    })
    it('overpay one for zero and exact in', async () => {
      await expect(
        underpay.swap(await pool.getAddress(), await wallet.getAddress(), false, MAX_SQRT_RATIO - 1n, 1000, 0, 2000)
      ).to.not.be.revertedWith('IIA')
    })
