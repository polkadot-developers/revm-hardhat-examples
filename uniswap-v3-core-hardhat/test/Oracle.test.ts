...
describe('Oracle', () => {
  ...
  async function deployOracle(): Promise<OracleTest> {
    const oracle = (await ethers.deployContract('OracleTest')) as unknown as OracleTest
    await oracle.waitForDeployment()
    return oracle
  }

  async function deployInitializedOracle(): Promise<OracleTest> {
  ...
  describe('#grow', () => {
    ...
    beforeEach('deploy initialized test oracle', async () => {
      oracle = await deployInitializedOracle()
    })
  ...
  describe('full oracle', function () {
    ...
    beforeEach('create a full oracle', async function () {
      oracle = await deployOracle()
      ...
      while (cardinalityNext < 65535) {
        const growTo = Math.min(65535, Number(cardinalityNext) + BATCH_SIZE)
        console.log('growing from', cardinalityNext, 'to', growTo)
        await oracle.grow(growTo)
        cardinalityNext = BigInt(growTo)
      }
    ...
    async function checkObserve(
      secondsAgo: number,
      expected?: { tickCumulative: bigint | string; secondsPerLiquidityCumulativeX128: bigint | string }
    ) {
